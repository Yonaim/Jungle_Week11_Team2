...
void FLevelEditorWindow::RequestDefaultDockLayout()
{
	...
	{
		// Asset editor side handles its own default layout. Do NOT set Level's
		// bPendingDefaultDockLayout here — it is a cross-concern flag that would
		// make ApplyPendingDefaultDockLayout wake up the next time Level becomes
		// active, even though Level's layout never needed rebuilding.
...
void FLevelEditorWindow::RequestRestoreForActiveDocument()
{
	...
	if (FLevelDocumentTab* Tab = GetActiveLevelDocumentTab())
	{
		// Asset→Level 전환을 거치면 ImGui가 새 MainDockspaceId 해시 트리를
		// reactivate하는 데 2-3 frame이 걸려 captured DockId가 한 frame 동안
		// stale로 보일 수 있다. 4 frame 동안 SetNextWindowDockID(captured, Always)
		// 를 재호출해 안전 margin 확보. Level↔Level 전환에도 동일하게 적용되지만
		// 비용은 무시 가능.
		FPanel::RequestCapturedLayoutRestore(&Tab->LayoutState, 4);
	}
}
	...
	if (ImGui::Begin("##EditorDocumentTabBar", nullptr, Flags))
	{
		...
		if (Result.SelectedIndex >= 0 && Result.SelectedIndex != ActiveUnifiedIndex)
		{
			if (Result.SelectedIndex < LevelTabCount)
			{
				...
				// 타협안 (Phase B): captured DockId 복원이 ImGui dock tree reactivation
				// race로 floating panel을 만들어 내므로, tab 클릭 시점에 무조건 default
				// layout을 재구성한다. SetActiveLevelDocumentTab과
				// RestoreLevelEditorUIAfterAssetEditor가 호출한
				// RequestCapturedLayoutRestore는 PanelDockIds가 비워지면서 no-op이 됨.
				RequestDefaultDockLayout();
			}
			else
			{
				...
				if (bSwitchedTab)
				{
					...
					// Asset 측도 대칭적으로 default layout 강제. 이 시점에는 context가
					// Asset이므로 RequestDefaultDockLayout이 asset branch
					// (LevelEditorWindow.cpp:448-456) 를 타고 RequestDefaultLayoutForActiveTab
					// 으로 forward됨.
					RequestDefaultDockLayout();
				}
			}
...
void FLevelEditorWindow::RenderContent(float DeltaTime)
{
	...
	const float          OuterPadding = GetWindowOuterPadding();
	const float          DocumentTabBarHeight = GetDocumentTabBarHeight();
	// Re-read context LIVE: RenderDocumentTabBar() above may have flipped the context
	// (e.g., user clicked a Level tab while an Asset editor was active). Using the
	// pre-RenderDocumentTabBar cached value would leave Level panels unrendered for
	// one frame on the transition — causing dock state to drift in ImGui's internal
	// window tracking. The Early variant is intentionally preserved for callers that
	// require the pre-tab-bar snapshot.
	(void)bAssetEditorContextActiveEarly;
	...
	if (ImGui::Begin("##EditorDockSpaceHost", nullptr, DockspaceWindowFlags))
	{
		...
		// and the last captured DockId.
		// Asset 활성 중에는 LEVEL의 모든 tab을 keep-alive로 제출해야 한다 — 실제 panel
		// 렌더 경로 (line 1316+) 가 !bAssetEditorContextActive로 가드되어 어느 level
		// tab도 panel을 제출하지 않으면 ImGui가 panel→dock-node binding을 잃을 수 있다.
		// Level 활성 중에는 active tab만 실제 panel로 렌더되므로 나머지만 keep-alive.
		const bool bAssetActiveForKeepAlive = bAssetEditorContextActive;
		for (FLevelDocumentTab& Tab : LevelDocumentTabs)
		{
			...
			const bool bTabIsActive = (Tab.LayoutId == ActiveLayoutId);
			if (bTabIsActive && !bAssetActiveForKeepAlive)
			{
				continue;
			}
...
void FLevelEditorWindow::RestoreLevelEditorUIAfterAssetEditor()
{
	...
	if (FLevelDocumentTab* Tab = GetActiveLevelDocumentTab())
	{
		...
		{
			// Asset→Level 전환 시 ImGui dock tree reactivation race에 대응하기 위해
			// 4 frame 동안 captured DockId 재적용 (Panel.h:145 기본 2 frame은 경계).
			FPanel::RequestCapturedLayoutRestore(&Tab->LayoutState, 4);
		}
	}
}
